...
class ResNetModel(nn.Module):
    def __init__(self, maxlen, max_features, embedding_dims, class_num):
        ...
        self.conv = nn.Conv1d(768, 32, kernel_size=8, stride=1, padding=4)
    ...
    def forward(self, x):
        ...
        output_tensor1 = output_tensor.permute(0, 2, 1)
        # 输出张量的形状
        # print(output_tensor.shape)
        # processed_sequence = ' '.join(k_mers)
        # model_input = tokenizer.encode_plus(processed_sequence, add_special_tokens=False, max_length=512)["input_ids"]
        # model_input = torch.tensor(model_input, dtype=torch.long)
        # model_input = model_input.unsqueeze(0)   # to generate a fake batch with batch size one
        # output = model(model_input)
        #embedding = self.embedding(x)
        #embedding1 = embedding.permute(0, 2, 1)
        conv_out = self.conv(output_tensor1)
        conv_out = self.bn(conv_out)
        conv_out = F.relu(conv_out)
        conv_out = F.max_pool1d(conv_out, kernel_size=2, stride=1)
        resnet_out = resnet_convolutional_block(conv_out, 64, 8)
        resnet_out = resnet_identity_block(resnet_out,64, 8)
        resnet_out = resnet_identity_block(resnet_out,64, 8)
        resnet_out = self.global_max_pool(resnet_out).squeeze(2)
        lstm_out, _ = self.lstm(output_tensor)
        ...
        fusion_out = torch.cat((lstm_out, resnet_out), dim=1)
        fc_out = F.relu(self.fc(fusion_out))
...
if __name__ == '__main__':#第一层
    ...
    # checkpoint_filepath = 'best_model919.h5'
    ...
    for epoch in range(10):
        ...
        epoch_progress_bar = tqdm(train_loader, desc=f'Epoch {epoch + 1}/{10}', leave=True)
        ...
        # 在验证集上计算损失
        model_layer1.eval()  # 设置模型为评估模式
        val_loss = 0.0
        with torch.no_grad():
            for val_inputs, val_labels in val_loader:
                val_inputs = val_inputs.to(device)
                val_labels = val_labels.to(device)
                val_outputs = model_layer1(val_inputs)
                val_loss += loss_function(val_outputs, val_labels.float()).item() * val_inputs.size(0)
        val_loss /= len(val_dataset)

        # 保存损失最小的模型权重
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model_layer1, 'best_model_10.pth')

    # 保存最后一个 epoch 的模型权重
    torch.save(model_layer1, 'final_model_10.pth')
